...
getTopicFromNoteSettings <- function(connection,
    ...
    sql <- paste(
        ...
        'NOTE_TEXT AS covariate_id,',
        '1 AS covariate_value',
        'FROM @cdm_database_schema.NOTE',
        'WHERE NOTE_TYPE_CONCEPT_ID = @note_concept_id',
        'AND NOT NOTE_DATETIME IN (SELECT NOTE_DATETIME FROM @cdm_database_schema.NOTE n JOIN @cohort_table c ON n.person_id = c.subject_id AND n.NOTE_DATE = c.cohort_start_date',
    ...
    if(covariateSettings$useTopicModeling == TRUE){
        ...
        rowTotals <- apply(data , 1, sum)
        dtm.new   <- data[rowTotals> 0, ]

        best.model <- lapply(seq(2,100, by=1), function(k){LDA(dtm.new, k)})
        ...
        lda_model = text2vec::LDA$new(n_topics = optimal, doc_topic_prior = 0.1, topic_word_prior = 0.01) # covariateSettings$numberOfTopics -> optimal
